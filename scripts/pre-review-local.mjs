...
function classificationFromInputs({ branch, message }) {
...
function scopeVerdictFor(classification) {
...
function scanForBlockedDiffPatterns(base, changedFiles) {
  ...
  const diffChunks = sourceFiles
    .map((file) => {
      const result = runCommand(
        `git diff ${base}...HEAD -- "${file.replaceAll('"', '\\"')}"`,
      );
      return result.ok ? result.stdout : "";
    })
    .join("\n");

  if (/\bany\b/.test(diffChunks)) {
    issues.push(
  ...
  for (const pattern of secretLikeTokens) {
...
function collectChangedFiles(base) {
  const result = runCommand(`git diff --name-only ${base}...HEAD`);
  if (!result.ok) {
    return {
      files: [],
      lines: "",
...
function collectCommitMessage(base) {
  const result = runCommand(`git log -1 --pretty=%s ${base}..HEAD`);
  if (result.ok && result.stdout.trim()) return result.stdout.trim();
  const fallback = runCommand("git log -1 --pretty=%s HEAD");
...
function runChecks() {
  ...
  const branch =
    runCommand("git branch --show-current || true").stdout.trim() ||
    "HEAD (detached)";
