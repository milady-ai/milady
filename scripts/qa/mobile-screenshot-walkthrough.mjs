...
const STEPS = [
  {
    ...
    notes: 'iOS: open apps/app/ios/App.xcworkspace, run on iPhone 15 sim. Android: `npx cap run android`.',
  },
  {
    id: 'M2',
  ...
  },
  {
    ...
    notes: 'See packages/ui/src/onboarding/mobile-runtime-mode.ts. Android pre-seeds a local agent before the provider step.',
  },
  {
    id: 'M4',
    file: 'M4-deep-link-provider.png',
    title: 'Deep link entry — milady://onboard/step/provider',
    notes: 'Trigger the deep link from the sim/emu shell, then screenshot the provider step.',
  },
  {
    id: 'M5',
    file: 'M5-permission-prompt.png',
    title: 'Native permission prompt (notifications / file access)',
    notes: 'Playwright cannot reach native dialogs — this is the reason computer-use MCP is required.',
  },
];
...
function writeChecklist(surface, dir) {
  ...
  for (const step of STEPS) {
    lines.push(`| ${step.id} | [ ] | \`${step.file}\` | ${step.title} | ${step.notes} |`);
  }
  lines.push('');
  lines.push('## Capture commands (operator)');
  lines.push('');
  lines.push('Each PNG must be saved to this directory with the filename listed above. Example MCP call:');
  lines.push('');
  lines.push('```');
  ...
  lines.push('');
  lines.push('```');
  lines.push(`node scripts/qa/mobile-screenshot-walkthrough.mjs --finalize --surface ${surface}`);
  lines.push('```');
  lines.push('');
...
function init(surface) {
  ...
  process.stdout.write(`Scaffolded ${surface} mobile walkthrough at:\n  ${dir}\n`);
  process.stdout.write(`Checklist: ${join(dir, 'CHECKLIST.md')}\n`);
  process.stdout.write('Save screenshots into that directory using the filenames listed in CHECKLIST.md.\n');
  for (const step of STEPS) {
...
function finalize(surface) {
  ...
  lines.push('');
  lines.push('| Step | Expected file | Present | Size (bytes) | sha256 | Notes |');
  lines.push('|------|---------------|---------|--------------|--------|-------|');
  for (const step of STEPS) {
    const hit = present.find((p) => p.step.id === step.id);
    if (hit) {
      lines.push(`| ${step.id} | \`${step.file}\` | yes | ${hit.size} | \`${hit.sha256}\` | ${step.title} |`);
    } else {
      lines.push(`| ${step.id} | \`${step.file}\` | no  | — | — | ${step.title} |`);
    }
  }
  ...
  const extras = [];
  try {
    const expected = new Set(STEPS.map((s) => s.file).concat(['CHECKLIST.md', 'SUMMARY.md']));
  ...
  if (missing.length === 0) {
    process.stdout.write(`PASS: ${present.length}/${STEPS.length} steps captured\n`);
    return;
  }
