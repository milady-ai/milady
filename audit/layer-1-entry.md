...
- The bug from MASTER.md §0 was a Layer 1 issue — `LOCAL_AGENT_API_BASE`
  hardcoded in the retired runtime-selection gate (Layer 7) ignoring what Layer 1
...
- [!] `eliza/packages/app-core/src/first-run-config.ts` — pure runtime config builder. It still has too many optional fields; the next cleanup should replace the broad args shape with discriminated local/cloud/remote plans used by first-run setup.
...
- `window.__ELIZA_API_BASE__` (legacy global API-base read)
...
| `apps/app/src/main.tsx` line 21 — `const PhoneCompanionApp = () => null;` | 1 | **Inline fork-leftover stub.** Comment (lines 17-21) admits "PhoneCompanionApp is mobile-companion-only and not exported by the current `@elizaos/app-core` surface." | The upstream `@elizaos/app-core` in `elizaOS/eliza` doesn't export `PhoneCompanionApp`; no equivalent exists upstream. | **Decide:** either land the export upstream (preferred), or move the stub to `apps/app/src/stubs/phone-companion.tsx` so it's visible alongside the others. Inline `() => null` in the entry file is the worst location. |
...
|--------|-------------------|------|-------|
| `window.__ELIZA_API_BASE__` | `electrobun/src/index.ts:843, 853` (HTML inject) | `apps/app/src/main.tsx:190` (`getInjectedAppApiBase`); old reader was the retired runtime-selection gate (Layer 7, the bug source per MASTER.md §0) | **The bug.** Two writers (HTML inject + RPC push), one renderer-side reader that locked. |
...
5. **Move `eliza/packages/app-core/src/first-run-config.ts` under `./first-run/`** and replace its 23-optional-field interface with a discriminated union per provider/runtime target. Currently it's both mislocated (entry-layer-adjacent file with deep first-run logic) and weakly-typed.
