...
1. `first-run/` (7 files, ~610 LOC) — pure first-run-flow resolution + the four mobile/local-runtime helpers (probe, pre-seed, mobile mode, picker reload, server-target enum, native local-agent token).
...
- First-run consumes Layer 8's `state/persistence.ts` for resume/step
  storage, Layer 8's startup-coordinator for the runtime-target events,
  and Layer 8's `useFirstRunState` / `useFirstRunCallbacks` hooks
...
- **First-run bootstrap** — the fresh-install → "lands in chat" flow.
  The user-facing flow is `first-run.ts` plus the shell screen; the internal
  setup step helper is `setup-steps.ts`.
- **Probe local agent edge cases** — `probe-local-agent.ts` is the
  async readiness signal for local first-run setup.
- **Three near-duplicate `tryLocalStorage` patterns** — `mobile-runtime-mode.ts`, `pre-seed-local-runtime.ts`, and the retired runtime-switch helper each inline a try/catch around localStorage; same block as Layer 8's `state/persistence.ts:25-32` and `state/agent-profiles.ts:18-25`. Five copies total.
...
### A. First-run flow diagram (fresh install → "lands in chat")
...
Fresh install
       ...
       │                   ├─ if firstRunComplete → starting-runtime → ready
       │                   └─ else → first-run-required
       │                            ↓
      │                          [first-run screen runs:
      │                            first-run/first-run.ts owns draft/payload
      │                            setup-steps.ts remains internal state glue]
       │                            → FIRST_RUN_COMPLETE → starting-runtime → hydrating → ready
       │
       └─ [AOSP ElizaOS APK]
  ...
  Use Local from first-run setup:
       │
       ├─ first-run controller resolves the API base from getElizaApiBase()
...
|---|-------------------------------------------------------|-------------------------------------------------------------------------|--------------------------------------------------------|
| 12 | First-run-complete flag → vault prefs               | `state/persistence.ts:359-377` (4 callers; see Layer 8 audit)           | **Ready to implement.** Move 2 functions, drop key from `bridge/storage-bridge.ts:SYNCED_KEYS`, delete legacy alias. ~1 hour of work. |
| 13 | "Use local" atomic + actually disconnects cloud      | `useFirstRunCallbacks.ts` `finishAsLocal()` (Layer 8 hook, ~1125 LOC) | **Blocked on Layer 8 deep audit of `useFirstRunCallbacks.ts`.** The flow path is clear (above), but `useFirstRunCallbacks` is 1125 LOC and contains the cloud-disconnect side effects. Need to verify the cloud-clear ops are unconditional + execute before the local-mode write — today the order isn't guaranteed atomic. |
...
1. **Internal setup callbacks:** remaining work is collapsing unused state callbacks now that first-run is direct.
...
4. **The 5-copy `tryLocalStorage` pattern:** `state/persistence.ts:25-32`, `state/agent-profiles.ts:18-25`, plus inline blocks in `pre-seed-local-runtime.ts`, the retired runtime-switch helper, and `mobile-runtime-mode.ts`. Promote one helper to `utils/storage.ts` and replace.
5. **The 3-copy `ACTIVE_SERVER_STORAGE_KEY` literal:** `state/persistence.ts`, `pre-seed-local-runtime.ts:33`, and the retired runtime-switch helper. The current dedup is *deliberate* (cycle avoidance). Resolution: extract to `state/storage-keys.ts` (a leaf-only constants file with zero imports) and have all three import from there.
...
| `first-run/local-agent-token.ts:68-69`       | Dual write to typed boot-config slot AND legacy `__ELIZA_API_TOKEN__` window global | Symptom of the Layer 1 finding — boot-config-store should be the only writer; window-global should be deleted across the codebase. |
| `first-run/setup-steps.ts` callbacks | Still threaded through state for internal completion paths | Collapse once all callers use direct first-run submit plans. |
...
import-path collisions and first-run confusion ("which bridge?").
