...
function computeTrustScore(history, config = DEFAULT_CONFIG, now = Date.now()) {
  ...
  for (const event of sorted) {
    ...
    if (event.type === 'reject' && event.reviewSeverity) {
      severityMultiplier = config.reviewSeverity[event.reviewSeverity]
    ...
    if (basePoints >= 0) {
      ...
      eventPoints = basePoints * diminishingMultiplier * recencyWeight
        * complexityMultiplier * categoryMultiplier * streakMult;
    } else {
      ...
      eventPoints = basePoints * recencyWeight * severityMultiplier
        * streakMult * Math.max(categoryMultiplier, 0.8); // floor category at 0.8 for penalties
    }
    ...
    if (eventPoints > 0) {
      ...
      if (capped < eventPoints) {
        ...
        warnings.push(`Daily cap hit on ${dateKey}: PR #${event.prNumber} capped from ${round(eventPoints, 2)} to ${round(capped, 2)}`);
      }
  ...
  if (recentPRs > config.velocity.hardCapPRs) {
    ...
    warnings.push(`VELOCITY HARD CAP: ${recentPRs} PRs in ${config.velocity.windowDays} days (limit: ${config.velocity.hardCapPRs})`);
    ...
    warnings.push(`Velocity warning: ${recentPRs} PRs in ${config.velocity.windowDays} days (soft cap: ${config.velocity.softCapPRs})`);
  }
...
function expandState(compact) {
  ...
  const severityMap = { c: 'critical', m: 'major', n: 'normal', i: 'minor', t: 'trivial' };
...
function runExamples() {
  ...
  printScenario('Scenario 1: Steady Eddie', '10 approvals over 90 days, growing complexity', steadyEddie, NOW);
  ...
  printScenario('Scenario 2: Speed Demon', '8 trivial chore PRs in 3 days (gaming)', speedDemon, NOW);
  ...
  const securityHero = {
    ...
    events: [
      { type: 'approve', timestamp: NOW - 50 * DAY, linesChanged: 200, labels: ['security'], prNumber: 300 },
      { type: 'approve', timestamp: NOW - 30 * DAY, linesChanged: 350, labels: ['security', 'critical-fix'], prNumber: 301 },
      { type: 'approve', timestamp: NOW - 10 * DAY, linesChanged: 150, labels: ['security'], prNumber: 302 },
    ],
  };
  printScenario('Scenario 3: Security Hero', '3 security fixes over 2 months', securityHero, NOW);
  ...
  const roughStart = {
    ...
    createdAt: NOW - 60 * DAY,
    events: [
      { type: 'reject', timestamp: NOW - 55 * DAY, linesChanged: 100, labels: ['feature'], prNumber: 400, reviewSeverity: 'major' },
      { type: 'reject', timestamp: NOW - 50 * DAY, linesChanged: 80, labels: ['feature'], prNumber: 401, reviewSeverity: 'normal' },
      { type: 'close', timestamp: NOW - 48 * DAY, linesChanged: 200, labels: ['feature'], prNumber: 402 },
      { type: 'approve', timestamp: NOW - 40 * DAY, linesChanged: 60, labels: ['bugfix'], prNumber: 403 },
      { type: 'approve', timestamp: NOW - 30 * DAY, linesChanged: 120, labels: ['bugfix'], prNumber: 404 },
      { type: 'approve', timestamp: NOW - 20 * DAY, linesChanged: 200, labels: ['feature'], prNumber: 405 },
      { type: 'approve', timestamp: NOW - 10 * DAY, linesChanged: 180, labels: ['feature'], prNumber: 406 },
      { type: 'approve', timestamp: NOW - 5 * DAY, linesChanged: 250, labels: ['core'], prNumber: 407 },
    ],
  };
  printScenario('Scenario 4: Rough Start', '3 rejections then 5 approvals (redemption arc)', roughStart, NOW);
  ...
  printScenario('Scenario 5: Gone Ghost', '8 approvals but last activity 120+ days ago', goneGhost, NOW);
  ...
  printScenario('Scenario 6: Typo Farmer', '15 tiny doc PRs over 30 days (gaming via volume)', typoFarmer, NOW);
...
function printScenario(name, description, history, now) {
  ...
  console.log(`  Weighted points sum: ${result.breakdown.recencyWeightedPoints}`);
  console.log(`  Velocity penalty:    ${(result.breakdown.velocityPenalty * 100).toFixed(1)}%`);
