...
- **Runtime config**: `~/.milady/milady.json` (or `ELIZA_CONFIG_PATH` / `ELIZA_STATE_DIR`)
- **Env secrets**: `~/.milady/.env` or project `.env`
- **Namespace**: The CLI auto-detects "milady" from package.json name — state dir is `~/.milady/`, config file is `milady.json`
...
- **Config file not found**: The actual path is `~/.milady/milady.json` (namespace is "milady", not "eliza"). Do not look in `~/.eliza/`.
