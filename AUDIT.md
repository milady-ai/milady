...
| 9     | First-run + bridge                             |    15 |    15   |     0      | partial  |
...
Layer 0 ─→ Layer 1 ─→ Layer 5 (vault, shared, ui)
              ...
              Layer 2 (Electrobun)   Layer 3 (runtime) ─→ Layer 4 (api)
                ↓                        ↓                    ↓
                └────────────────────────┴────── Layer 8 (state/config) ─→ Layer 9 (first-run) ─→ Layer 7 (UI)
                                                       ↓
...
- [Layer 9 — First-run + bridge](./audit/layer-9-first-run-bridge.md) — 15/15 audited; Phase 2 task 12 ready, task 13 blocked on Layer 8 hook deep audit, task 14 ~70% done
...
```
- [!] eliza/packages/app-core/src/components/shell/retired-runtime-gate.tsx
