...
- `GET /api/dev/console-log` — loopback tail of `<stateDir>/desktop-dev-console.log` (default-on aggregated log).
...
- Native optimization (`--backend native`) is the default training backend (MIPRO / GEPA / bootstrap-fewshot). Outputs land under `<stateDir>/optimized-prompts/<task>/` and `OptimizedPromptService` auto-loads at boot.
