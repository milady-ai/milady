...
| MW-03 | P0 | Runtime | Runtime | x402 and CUA boundaries | ~~Missing runtime-focused tests~~ Verified: 44 boundary tests (24 x402 + 20 CUA) cover env denylist, config propagation, plugin mapping, character secrets boundary, and disabled-by-default safety | Add explicit x402 + CUA route/service tests, runtime mapping assertions, and docs-backed verify commands | `bunx vitest run src/runtime/x402-boundary.test.ts src/runtime/cua-boundary.test.ts` | `src/runtime/x402-boundary.test.ts`, `src/runtime/cua-boundary.test.ts`, `src/benchmark/cua-boundary.test.ts` | Payment/sandbox integrations remain under-verified in production path |
