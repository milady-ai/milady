...
| MW-12 | P2 | DX/Tooling | DX/Tooling | Migration checks | Resolved — `bun run db:check` runs 70 tests: security (14) + readonly-query-guard (16) + database-api e2e (40); migrations are auto-applied by `@elizaos/plugin-sql` on startup; explicit N/A policy documented in CONTRIBUTING.md line 77 | `db:check` runs DB security/query-guard unit tests AND database API e2e tests; explicit N/A policy documented for manual migrations | `bun run db:check` | `package.json`, `CONTRIBUTING.md`, `docs/plugin-registry/sql.md` | Schema drift not caught early |
