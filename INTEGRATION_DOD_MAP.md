...
| MW-12 | P2 | DX/Tooling | DX/Tooling | Migration checks | Verified — `bun run db:check` runs 70 tests (30 unit + 40 e2e); CI enforces in `.github/workflows/test.yml`; N/A policy for manual migrations documented in `CONTRIBUTING.md` and `docs/plugin-registry/sql.md`; agent-review.yml requires it for DB changes | `db:check` runs DB security/query-guard unit tests AND database API e2e tests; explicit N/A policy documented for manual migrations | `bun run db:check` | `package.json`, `docs/plugin-registry/sql.md`, `CONTRIBUTING.md`, `.github/workflows/test.yml` | Schema drift not caught early |
