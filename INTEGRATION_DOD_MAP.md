...
| MW-04 | P0 | Security | Security | Websocket/auth surfaces | ~~Ensure all sensitive ingress endpoints have explicit auth tests~~ Resolved: 108 auth tests across 5 files — WebSocket origin/token auth (22), MCP config validation with blocked commands/env/hosts (34), wallet export token auth (12), config/secrets mutation auth gates (26), MCP config auth gates (14) — both accept and reject paths verified for all sensitive ingress | Add/extend auth tests for config mutation, secrets update, connector mutation, MCP mutation | `bunx vitest run src/api/server.websocket-auth.test.ts src/api/server.mcp-config-validation.test.ts src/api/server.wallet-export-auth.test.ts src/api/server.config-secrets-auth.test.ts src/api/server.mcp-config-auth.test.ts` | `src/api/server.*.test.ts` | Unauthorized access vectors |
