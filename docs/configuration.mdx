...
description: "Complete reference for the Milaidy config file, environment variables, CLI commands, and all configuration sections."
---

Milaidy uses a single JSON5 config file for all settings. This page documents every configuration section, environment variable override, and CLI command available.

## Config File Location

By default, Milaidy stores its configuration at:
...
<Info>
The config directory is `~/.milady` and the config file is `milady.json` inside it. Despite the directory name, the product is **Milaidy** and the CLI binary is `milaidy`.
</Info>

### Overriding the Location

Two environment variables control where Milaidy looks for configuration:
...
```bash
# Use a custom state directory (config lives at /opt/milaidy/milady.json)
MILADY_STATE_DIR=/opt/milaidy milaidy start

# Point to a specific config file
MILADY_CONFIG_PATH=~/projects/bot/config.json5 milaidy start
```
...
Milaidy loads environment variables from two sources:
...
Plugins extend Milaidy with connectors, providers, and feature capabilities.
...
Milaidy automatically enables plugins based on your configuration. You do not need to manually add plugins to the allowlist in most cases.
...
| `whatsapp` | `@elizaos/plugin-whatsapp` | `connectors.whatsapp.authDir` |
| `signal` | `@elizaos/plugin-signal` | `connectors.signal.account` |
...
| `GOOGLE_API_KEY` / `GOOGLE_GENERATIVE_AI_API_KEY` | `@elizaos/plugin-google-gemini` |
...
Connectors integrate Milaidy with messaging platforms. Configure them under `connectors`:
...
<Info>
When no config file exists, Milaidy defaults to `logging.level: "error"`.
</Info>

## Database

Milaidy supports two database backends:
    ...
    ```json5
    {
      database: {
        ...
        postgres: {
          ...
          connectionString: "postgresql://user:pass@localhost:5432/milaidy",
...
Key environment variables that affect Milaidy behavior:
...
|----------|---------|---------|
| `MILAIDY_GATEWAY_PORT` | `18789` | Gateway (API + WebSocket) port |
| `MILAIDY_PORT` | `2138` | Dashboard (Web UI) port |
...
Milaidy provides CLI commands to manage configuration without editing JSON directly:
...
milaidy config get agents.defaults.model
milaidy config get gateway.port

# Set a config value
milaidy config set gateway.port 19000
milaidy config set agents.defaults.thinkingDefault "high"
```

<Tip>
Use `milaidy configure` for initial setup. It walks through model provider selection, agent personality, and connector setup interactively.
</Tip>
