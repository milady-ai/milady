...
- `~/.local/state/milady/milady.json` — primary Milady on-disk config. Written via `saveElizaConfig(...)` from `@elizaos/agent`. The resolver still reads `<stateDir>/eliza.json` as a legacy candidate when the namespace is not `eliza`.
...
- `<stateDir>` is resolved by `resolveStateDir()` (Milady default `~/.local/state/milady`; elizaOS default `~/.local/state/eliza`). `MILADY_STATE_DIR` and `ELIZA_STATE_DIR` are aliases; `ELIZA_STATE_DIR` wins when both are present.
