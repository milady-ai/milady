...
|  ~/.milady/milaidy.json  (Zod-validated config)              |
...
<Steps>
  <Step title="Load Configuration">
    Read `~/.milady/milaidy.json` via `loadMiladyConfig()`. If no config exists, fall back to an empty `MiladyConfig` object with all-optional fields.
  </Step>
...
Highest priority
  ...
  4. Config file (~/.milady/milaidy.json)
