...
|  ~/.milaidy/milaidy.json  (Zod-validated config)              |
|  PGLite (default) or PostgreSQL (database)                    |
|  ~/.milaidy/workspace/ (agent files, skills, bootstrap)       |
|  ~/.milaidy/plugins/  (custom, ejected, installed)            |
+---------------------------------------------------------------+
```

## Entry Points and Boot Sequence

Milaidy has two primary entry points, plus a dev-server variant:

### CLI Entry (`src/entry.ts`)

The CLI entry point is the bootstrap for all Milaidy operations. Built by `tsdown` into `dist/entry.js` and invoked by the `milaidy.mjs` wrapper script.

The boot sequence is:

1. **Set process title** to `milaidy` (visible in `ps` output)
...
<Steps>
  <Step title="Load Configuration">
    Read `~/.milaidy/milaidy.json` via `loadMiladyConfig()`. If no config exists, fall back to an empty `MiladyConfig` object with all-optional fields.
  </Step>
  ...
  <Step title="Build Character">
    Construct an ElizaOS `Character` object from the Milaidy config: agent name, bio, system prompt, style, adjectives, topics, message examples, and API key secrets.
  </Step>
  <Step title="Prepare Workspace">
    Ensure `~/.milaidy/workspace/` exists with bootstrap files. Create the `~/.milaidy/plugins/custom/` directory for drop-in plugins.
  </Step>
  <Step title="Create Milaidy Plugin">
  ...
  <Step title="Create AgentRuntime">
    Instantiate `AgentRuntime` with the character, Milaidy plugin, and all resolved plugins. Pre-register `plugin-sql` (with PGLite recovery) and `plugin-local-embedding` before other plugins to avoid race conditions.
  </Step>
...
Milaidy extends ElizaOS through several integration points:

### Plugin System Bridge

The `createMiladyPlugin()` function (`src/runtime/milady-plugin.ts`) creates a standard ElizaOS `Plugin` that bridges Milaidy-specific functionality into the runtime:
...
- Custom actions loaded from `~/.milaidy/workspace/custom-actions/`
...
- **Ejected plugins** (`~/.milaidy/plugins/ejected/`) -- override npm/core versions
- **Custom plugins** (`~/.milaidy/plugins/custom/`) -- user-created plugins
...
Milaidy inherits ElizaOS's three-part extension model:
...
Milaidy registers 8+ providers through the Milaidy plugin, plus any providers from loaded plugins.
...
Milaidy uses `@elizaos/plugin-sql` as the database adapter with two backend options:

| Backend | Config | Use Case |
|---------|--------|----------|
| **PGLite** (default) | `database.provider: "pglite"` | Zero-config embedded PostgreSQL. Data stored in `~/.milaidy/workspace/.eliza/.elizadb/` |
...
When `MILAIDY_API_TOKEN` is set (or auto-generated for non-loopback binds), all API requests must include a valid bearer token:

```
Authorization: Bearer <token>
```

If the API is bound to a non-loopback address (`MILAIDY_API_BIND` is not `127.0.0.1` or `localhost`) and no token is set, the server auto-generates a temporary token and logs it to stderr.
...
Milaidy's `src/services/` directory contains standalone service modules:
...
Milaidy resolves configuration through a priority cascade:

```
Highest priority
  1. CLI flags (--debug, --verbose, --no-color, --profile)
  2. Environment variables (MILAIDY_*, ANTHROPIC_API_KEY, etc.)
  3. .env file (loaded by dotenv in run-main.ts)
  4. Config file (~/.milaidy/milaidy.json)
...
The state directory (`~/.milaidy/`) is resolved via:

1. `MILAIDY_STATE_DIR` environment variable (if set)
2. Default: `~/.milaidy/` (`$HOME/.milaidy/`)

The config file path follows a similar pattern:
1. `MILAIDY_CONFIG_PATH` environment variable (if set)
2. Default: `<stateDir>/milaidy.json`

### Config File Structure

Milaidy uses a Zod-validated JSON configuration file (`milaidy.json`):
...
- Token-based auth via `MILAIDY_API_TOKEN` (bearer token on all `/api/*` requests)
...
- **Config allowlist** prevents API endpoints from writing sensitive env vars (e.g., `MILAIDY_API_TOKEN` is stripped from config patches)
...
|----------|----------|-------|
| CLI | `milaidy start` | Direct Node.js process, runner script handles restart exit codes |
...
Milaidy requires **Node.js 22+** (specified in `package.json` `engines` field). The `node-llama-cpp` dependency for local embeddings benefits from native compilation on the target platform.
