...
The current Milaidy version is resolved at import time via `src/runtime/version.ts`, which calls `resolveMiladyVersion()`. This function checks, in order:
...
The notification uses Milaidy's terminal theme for coloring: the current version is dimmed, the new version is highlighted in green, and the channel suffix (for non-stable channels) is shown in parentheses.
...
After a successful fetch, the checker persists the check metadata (`lastCheckAt`, `lastCheckVersion`) to `milaidy.json`. If the config file is unwritable, a warning is printed to stderr but the check result is still returned.
...
This is used by the `milaidy update status` command to display a complete version overview.
...
2. Saves the new channel to `milaidy.json`
...
Milaidy automatically detects how it was installed to determine the correct update command. The `detectInstallMethod()` function in `src/services/self-updater.ts` works by:

1. Running `which milaidy` to find the binary path (5-second timeout)
...
If `which milaidy` returns nothing (binary not in PATH), the detection falls back to checking whether the current directory is a local development checkout (by looking for `devDependencies` in the root `package.json`).
...
For Homebrew, the command is simply `brew upgrade milaidy` (Homebrew manages version resolution internally). The `apt` case wraps the update in `sh -c` since it requires two sequential commands.
...
<Steps>
  ...
  <Step title="Detect Install Method">
    Resolve the binary path and determine which package manager or system was used to install Milaidy.
  </Step>
  ...
  <Step title="Verify Installation">
    Run `milaidy --version` (10-second timeout) and parse the output with a regex that extracts semver strings including pre-release tags. Report the version transition.
  </Step>
...
After the update command completes, Milaidy verifies the installation by running `milaidy --version` and parsing the output. The regex `(\d+\.\d+\.\d+(?:-[\w.]+)?)` handles both stable versions (e.g., `2.0.0`) and pre-release tags (e.g., `2.1.0-beta.3`). The version output may include a prefix like `milaidy/2.0.0`, which the parser handles.
...
The `milaidy update` command updates the CLI package itself, which includes:

| Component | Updated? | Notes |
|-----------|----------|-------|
| CLI binary (`milaidy`) | Yes | The entry point script and all compiled TypeScript |
...
| User-installed plugins | No | Installed independently in `~/.milaidy/plugins/` |
| Configuration (`milaidy.json`) | No | Preserved across updates |
| Database (PGLite data) | No | Preserved in `~/.milaidy/workspace/` |
| Workspace files | No | Agent workspace is independent of the CLI version |

Core ElizaOS plugins (`@elizaos/plugin-*`) are resolved from `node_modules` at runtime. When you update Milaidy and the new version specifies different plugin versions in its `dependencies`, those plugins are updated as part of the npm/bun install process.

## Version Pinning

To pin Milaidy to a specific version and prevent automatic updates:
...
milaidy config set update.checkOnStart false
```
...
The `--force` flag on `milaidy update` bypasses the interval cache but still respects the configured channel. It does not bypass version pinning at the package manager level.

## Rollback

Milaidy does not have a built-in rollback command, but you can revert to a previous version using your package manager:
...
- **Database migrations** may have run forward during the newer version. Downgrading could leave the database in an incompatible state. Milaidy's PGLite recovery mechanism will attempt to reset and rebuild the database if it detects corruption.
...
- `milaidy update --check` reports the network error and exits with code 1.
- `milaidy update` reports the error and exits with code 1.
...
# Transfer milaidy-2.0.0.tgz to the air-gapped machine, then:
npm install -g ./milaidy-2.0.0.tgz
```
...
When a new Milaidy version introduces breaking changes:

- **Database schema** -- Milaidy sets `ELIZA_ALLOW_DESTRUCTIVE_MIGRATIONS=true` by default, allowing the migration system to drop and recreate tables that have changed between plugin versions. This prevents the runtime from stalling on schema mismatches.
...
- **Config file** -- The config schema is forward-compatible. Unknown fields produce warnings but do not prevent startup. The config file tracks which Milaidy version last wrote it.
