...
description: Complete reference for all Milaidy REST API endpoints, organized by resource group.
---

The Milaidy API server runs on port **2138** by default. All endpoints are prefixed with `/api/`. Authentication is required when `MILAIDY_API_TOKEN` is set.

<Warning>
When `MILAIDY_API_TOKEN` is configured, include the token as a `Bearer` token in the `Authorization` header, or use the pairing flow to obtain it.
</Warning>
