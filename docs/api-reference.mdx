...
The Milaidy API server runs on port **2138** by default. All endpoints are prefixed with `/api/`. Authentication is required when `MILADY_API_TOKEN` is set.
...
```bash
curl "http://localhost:2138/api/knowledge/search?q=what+is+milaidy&limit=5" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
