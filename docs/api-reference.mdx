...
The Milady API server runs on port **2138** by default (production via `milady start`). In development mode (`bun run dev`), the API runs on port **31337** (`MILADY_API_PORT`) while the dashboard UI stays on **2138** (`MILADY_PORT`). All endpoints are prefixed with `/api/`. Authentication is required when `MILADY_API_TOKEN` is set.
