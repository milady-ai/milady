...
<Card title="Model Providers" icon="brain" href="/plugin-registry/llm/openai">
  LLM integrations for OpenAI, Anthropic, Google Gemini, Google Antigravity, Groq, Ollama, OpenRouter, DeepSeek, xAI, Mistral, Cohere, Together, Qwen, Minimax, Pi AI, Perplexity, Zai, Vercel AI Gateway, and Eliza Cloud. Providers auto-enable when their API key is set in the environment or in an `auth.profiles` entry.
</Card>

<Card title="Platform Connectors" icon="plug" href="/plugin-registry/platform/discord">
  Bridges to 30+ messaging platforms. **20 auto-enable** when configured in `connectors`: Discord, Telegram, Twitter, Slack, WhatsApp, Signal, iMessage, BlueBubbles, Blooio, MS Teams, Google Chat, Mattermost, Farcaster, Twitch, WeChat, Feishu, Matrix, Nostr, Lens, and Retake. **Additional connectors** available via manual install from the elizaOS registry: Bluesky, Instagram, LINE, Zalo, Twilio, GitHub, Nextcloud Talk, and Tlon.
</Card>
...
<Card title="Feature Plugins" icon="wand-magic-sparkles" href="/plugin-registry/browser">
  Extended capabilities — browser control, image generation (FAL), text-to-speech (Edge TTS, ElevenLabs), speech-to-text, computer use (CUA), cron scheduling, vision, shell, webhooks, Suno music, OpenTelemetry diagnostics, x402 payments, Obsidian vault integration, RepoPrompt, Claude Code workbench, Gmail Watch, personality, experience, and more. Feature plugins auto-enable via `features` flags or environment variables.
</Card>
...
3. **Core plugins** — Always loaded: `sql`, `local-embedding`, `form`, `knowledge`, `trajectory-logger`, `agent-orchestrator`, `cron`, `shell`, `agent-skills` (see `packages/agent/src/runtime/core-plugins.ts`). Optional core plugins — loaded when their feature flags, environment variables, or admin panel toggles are configured: `pdf`, `browser`, `computeruse`, `cua`, `obsidian`, `code`, `repoprompt`, `claude-code-workbench`, `vision`, `cli`, `discord`, `telegram`, `twitch`, `edge-tts`, and `elevenlabs`.
