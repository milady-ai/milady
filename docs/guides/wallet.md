...
**Planned:** optional storage in the OS secret store (macOS Keychain, Windows Credential Manager, Linux Secret Service) with a **per–state-directory vault id**, so keys are not persisted in plaintext config when enabled. See [Platform secure store (design)](./platform-secure-store.md). **macOS Node today:** Keychain writes go through the `security` CLI with `-w` on **argv** (momentary visibility in `ps`); Linux uses **stdin**. **Why it matters** and follow-up (Security.framework / FFI): [macOS Node implementation — argv exposure](./platform-secure-store.md#macos-keychain-cli-argv).
