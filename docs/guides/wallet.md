...
**Planned:** optional storage in the OS secret store (macOS Keychain, Windows Credential Manager, Linux Secret Service) with a **per–state-directory vault id**, so keys are not persisted in plaintext config when enabled. See [Platform secure store (design)](./platform-secure-store.md). **macOS Node today:** Keychain **writes** use a small **Swift** helper (`milady-keychain-store`) that reads the secret on **stdin** (Security.framework); **reads** still use `security find-generic-password`. Linux uses **`secret-tool` stdin**. Details: [macOS Node implementation](./platform-secure-store.md#macos-keychain-cli-argv).
