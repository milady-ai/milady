...
When `MILADY_API_TOKEN` is configured, the WebSocket upgrade request must include the token. The server calls `resolveWebSocketUpgradeRejection()` to validate credentials before accepting the connection. Unauthenticated upgrade requests receive an HTTP 401 or 403 rejection.
