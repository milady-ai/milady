...
<Update label="2026-03-24" description="Security: Keychain argv note, screenshot Bearer-only auth" tags={["Security", "Docs"]}>
## Platform secure store — document macOS `security` CLI argv exposure

**Why:** `MacOSKeychainPlatformSecureStore.set` calls `security add-generic-password -w <secret>`. The secret is on the child **argv** briefly (visible in `ps` to same-user / root). Linux `secret-tool store` uses **stdin** instead.

**Docs:** [Platform secure store — macOS Node implementation](/guides/platform-secure-store#macos-keychain-cli-argv) (mitigations, read/delete behavior, follow-up: Security.framework / Bun FFI). **Code comment:** `packages/app-core/src/security/platform-secure-store-node.ts`. **Wallet doc** links the same anchor.
