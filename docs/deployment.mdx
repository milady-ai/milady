...
<Tabs>
  <Tab title="milaidy-gateway">
    ...
    services:
      milaidy-gateway:
        image: ${MILADY_IMAGE:-milaidy:local}
        environment:
          HOME: /home/node
          TERM: xterm-256color
...
|-------------|---------------|---------|
| `MILADY_CONFIG_DIR` | `/home/node/.milady` | Configuration files (`milaidy.json`), database, secrets |
...
MILADY_HOME_VOLUME="milaidy-home" ./docker-setup.sh
```
...
|----------|---------|-------------|
| `MILADY_IMAGE` | `milaidy:local` | Docker image name/tag to use |
