...
| M5 | Permission prompts | Allow notifications, file access when prompted | computer-use MCP — save screenshot as `M5-permission-prompt.png` under the same scaffolded `reports/qa/<date>/mobile/<surface>/` directory; Playwright CANNOT reach native dialogs | Each prompt observed and granted |
