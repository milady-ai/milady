...
| `MILADY_DISABLE_AUTO_API_TOKEN` | When set to `1`, suppresses automatic API token generation for non-loopback binds. Ignored for cloud-provisioned containers — they always receive a fallback token to prevent auth deadlocks. | (unset) |
