...
1. On startup, Milaidy scans your environment variables (from `~/.milady/.env`, shell environment, or `milaidy.json`).
...
You can also enable providers manually in `~/.milady/milaidy.json` under the `plugins` key:
...
Add keys directly in `~/.milady/milaidy.json`:
...
In `~/.milady/milaidy.json`, specify the model using `provider/model` format:
