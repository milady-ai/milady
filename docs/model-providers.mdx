...
Milaidy supports **17 model providers** out of the box. Providers are loaded as plugins and can be auto-enabled by setting the right environment variable, or explicitly configured in `milaidy.json`.

---

## Provider Reference

The table below is the complete list of supported providers, sourced from the auto-enable system in Milaidy's core.

<Note>
Providers marked **bundled** ship with every Milaidy install. Others are installed on demand the first time they are needed — no manual plugin install required.
</Note>
...
| [Google Gemini](https://ai.google.dev) | `@elizaos/plugin-google-gemini` | `GOOGLE_API_KEY` or `GOOGLE_GENERATIVE_AI_API_KEY` | No | Gemini Pro, Flash, Ultra. |
...
Milaidy uses an **auto-enable** system. You do not need to manually register provider plugins — just set the API key and Milaidy handles the rest.

### Auto-enable flow

1. On startup, Milaidy scans your environment variables (from `~/.milaidy/.env`, shell environment, or `milaidy.json`).
...
3. The plugin is installed on demand if not already present (installed to `~/.milaidy/plugins/installed/`).
4. The provider becomes available for model selection.

### Explicit plugin configuration

You can also enable providers manually in `~/.milaidy/milaidy.json` under the `plugins` key:
...
Add keys directly in `~/.milaidy/milaidy.json`:
...
<Warning>
When using the config file approach, your API keys are stored in plaintext in `milaidy.json`. The `.env` file approach keeps secrets separate from configuration and is easier to exclude from version control.
</Warning>
...
```bash
milaidy models             # list configured model providers and their status
milaidy models add         # add a new provider interactively
milaidy models test        # test if your API keys are valid
milaidy configure          # interactive config wizard (includes provider setup)
```

### Setting the default model

In `~/.milaidy/milaidy.json`, specify the model using `provider/model` format:
...
Milaidy supports an ordered fallback chain. If the primary model fails (rate limit, outage, billing issue), the next model in the list is tried automatically.
