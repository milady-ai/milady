...
| [Google Vertex AI](https://cloud.google.com/vertex-ai) | `@elizaos/plugin-google-antigravity` | `GOOGLE_CLOUD_API_KEY` | No | Google Cloud / Vertex AI models (plugin codename: "Antigravity"). |
...
```bash
milady models             # show configured model providers and their status
milady configure          # interactive config wizard (includes provider setup)
milady setup --provider <name> --key <value>  # non-interactive provider setup
```
