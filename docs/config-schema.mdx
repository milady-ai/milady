...
Milaidy is configured through a `milaidy.json` file stored in the state directory (default: `~/.milady/milaidy.json`). All sections are optional; Milaidy uses sensible defaults when fields are omitted.
