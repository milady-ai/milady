...
description: Complete reference for the milaidy.json configuration file, organized by section with all fields documented.
---

Milaidy is configured through a `milaidy.json` file stored in the state directory (default: `~/.milaidy/milaidy.json`). All sections are optional; Milaidy uses sensible defaults when fields are omitted.
...
The `MiladyConfig` type defines every section available in `milaidy.json`. Below is the complete list of top-level keys:
...
|-------|------|---------|-------------|
| `lastTouchedVersion` | `string` | -- | Last Milaidy version that wrote this config |
...
| `lastRunVersion` | `string` | -- | Milaidy version of last wizard run |
...
| `color` | `string` | `#FF4500` | Accent color for the Milaidy browser profile (hex) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `seamColor` | `string` | -- | Accent color for Milaidy UI chrome (hex) |
...
| `basePath` | `string` | -- | Base path prefix (e.g. `"/milaidy"`) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `dataDir` | `string` | `~/.milaidy/workspace/.eliza/.elizadb` | PGLite data directory |
