...
Launch the TUI with the `milaidy tui` command. The `--port` flag overrides the default API port if the agent is running on a non-standard port.
...
milaidy tui --port 3000

# Connect to a remote agent
milaidy tui --api-base http://192.168.1.100:2138
```
...
- [CLI Reference](/cli/overview) — full list of `milaidy` CLI commands including `tui`
