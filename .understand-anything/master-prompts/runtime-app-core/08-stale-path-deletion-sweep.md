# Stale Runtime And First-Run Path Deletion Sweep
...
Sweep stale names and compat paths after the owning contracts are clear. This prompt is deletion-oriented but evidence-gated.
...
- `~/.milady`
- `onboarding` where it now means first-run setup
- duplicate runtime base selectors
- old splash/startup shell names

## Rules

- Delete stale paths when there is no live caller and no migration requirement.
- Rename docs/tests/scripts to current `FirstRunShell` and `~/.local/state/milady` contract.
- Do not keep v1/v2 bridges without a verified live caller.
- If a stale term remains, document the exact live reason.

## Verification

- `rg` sweep must show only intentional remaining references.
- Targeted tests for renamed contracts.
- Run `understand-diff` or update `.understand-anything/diff-overlay.json` after changes to see affected nodes.
