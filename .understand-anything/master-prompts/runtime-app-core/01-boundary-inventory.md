...
Produce a precise boundary inventory for Runtime App Core before implementation. Do not edit code in this pass unless you find a trivial documentation typo in generated reports.

## Questions To Answer

- Which files in `eliza/packages/app-core` own packaged runtime process launch?
- Which files own API server creation and health/readiness?
- Which files own state-dir/config resolution?
- Which files own Electrobun native RPC and preload/native bridge?
- Which app-core files import from `eliza/packages/ui`, and are those imports legitimate contracts or layer leaks?
- Which agent/core/shared imports are expected contracts versus avoidable barrel/hub coupling?
...
- Confirm every ownership claim points to a current file/function found by `rg`.
- Confirm the generated boundary inventory names the next implementation prompt.
- Do not change product source in this pass unless the prompt is explicitly expanded.

## Output

Write `.understand-anything/runtime-app-core-boundary-inventory.md` with:
