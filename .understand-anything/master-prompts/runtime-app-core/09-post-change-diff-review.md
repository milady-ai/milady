...
After a cleanup slice, use the graph to check whether the change stayed inside the intended architecture boundary.

## Commands

- `git diff --name-only`
- `node .understand-anything/tmp/build-architecture-debt-report.mjs`
- Re-run or refresh `understand-diff` overlay logic.

## Review Questions

- Which graph nodes changed?
- Which 1-hop affected nodes are outside Runtime App Core?
- Did cross-layer counts go down, stay flat, or increase?
- Did a high-degree hub become smaller or did we just move complexity?
- Did verification match the claim?

## Verification

- `git diff --name-only` matches the intended cleanup slice.
- Updated architecture debt report completes without validation issues.
- The review note calls out any verification gap plainly.
