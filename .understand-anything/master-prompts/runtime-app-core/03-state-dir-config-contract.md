# State Directory And Config Contract Cleanup
...
Collapse state-dir/config behavior into one explicit contract for Milady first-run and packaged desktop.

## Desired Contract

- Default state dir is `~/.local/state/milady`.
- `MILADY_STATE_DIR` and `ELIZA_STATE_DIR` aliases resolve predictably.
- First-run config writes to the same state root that app-core and desktop launch read.
- No stale `~/.milady` docs/scripts/tests unless deliberately documenting migration.

## Search First

- `rg -n "~/.milady|MILADY_STATE_DIR|ELIZA_STATE_DIR|stateDir|state-dir|resolveCompatConfigPaths|ELIZA_CONFIG_PATH|MILADY_CONFIG_PATH|firstRunComplete|first-run" .`

## Required Output

- Update tests before or with code changes.
- Add an explicit state-dir contract note if docs are stale.
- Report any legacy path you did not remove and why it still has a live caller.

## Verification

- Targeted tests around state-dir/config resolution.
- `rg -n "~/.milady|RuntimeGate|runtime=picker|DesktopOnboardingRuntime" .` shows only intentional remaining references.
- Fresh state-dir startup check if launch/config paths changed.
