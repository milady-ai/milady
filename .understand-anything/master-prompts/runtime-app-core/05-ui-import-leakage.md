...
Reduce `Runtime App Core -> Shared UI And First Run` coupling where app-core imports UI implementation instead of a stable contract. Do not break shared DTO/client contracts that are intentionally shared.

## Graph Evidence

The graph found 63 `Runtime App Core -> Shared UI And First Run` edges. Examples include app-core imports from UI transport/build-variant/platform modules.

## Search First

- `rg -n "from ['\"].*packages/ui|from ['\"]@elizaos/ui|ui/src|build-variant|ios-local-agent-kernel|mobile-runtime-mode|platform/index" eliza/packages/app-core`

## Desired Direction

- Shared contracts belong in `shared`, `core`, or an app-core-owned contract module.
- UI owns rendering and client-side state.
- app-core owns runtime, server, and platform policy.
- If an import exists only to reuse a tiny type or constant, move the type/constant inward instead of importing UI outward.

## Verification

- Typecheck the specific package(s) touched.
- Run targeted tests around moved contracts.
- Run `node .understand-anything/tmp/build-architecture-debt-report.mjs` after changes and compare the cross-layer count.
