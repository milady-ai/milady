# Packaged Startup Verification Master Prompt
...
Run the narrowest packaged-startup verification that gives a real signal for the current cleanup slice.

## Preconditions

- Do not claim success from typecheck/build alone.
- Use a fresh state dir for startup claims.
- Record exact ports/state dir/log findings.

## Baseline Commands To Adapt

- `node eliza/packages/app-core/scripts/desktop-build.mjs stage --variant=base --build-whisper`
- Fresh launch with `MILADY_STATE_DIR=$(mktemp -d /private/tmp/milady-runtime-core.XXXXXX)`, `ELIZA_STATE_DIR` set to the same path, explicit API/UI/screenshot ports, and `ELIZA_DIST_PATH=/Users/home/Documents/milady/dist`.
- Health checks: `/api/health`, `/api/first-run/status`, local ASR status route if relevant.
- Screenshot server if enabled.

## Verification

- Build/stage command exits successfully or reports a real blocker.
- Fresh launch uses a new state dir and reaches API readiness, or the exact failure is captured from logs.
- Renderer evidence comes from screenshot/console/log inspection, not assumption.

## Output

Report:

- Build/stage exit code.
- Fresh state dir.
- Whether API reached readiness.
- Whether any startup error occurred.
- Whether renderer showed first-run or main UI.
- Known remaining blockers, without overclaiming.
