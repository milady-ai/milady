# Electrobun Launch Contract Cleanup
...
Make packaged desktop runtime launch deterministic and owned by Electrobun/app-core. Focus on `eliza/packages/app-core/platforms/electrobun/src/native/agent.ts`, `eliza/packages/app-core/platforms/electrobun/src/index.ts`, remote runtime worker files, and packaged startup tests.

## Desired Contract

- Packaged desktop starts the sidecar/runtime from the packaged bundle, not a remote launcher.
- Sidecar env is explicit and testable.
- Startup should not block on local embedding/model downloads unless explicitly opted in.
- Structured remote/worker errors must never degrade to `[object Object]`.
- Renderer is informed of readiness; renderer does not choose process policy.

## Search First

- `rg -n "remote launch|remoteLaunch|sidecar|start\(|spawn|ELIZA_DIST_PATH|ELIZA_SKIP_LOCAL_EMBEDDING_WARMUP|ELIZA_ENABLE_STARTUP_LOCAL_EMBEDDING_WARMUP|\[object Object\]|ApiBridgeError" eliza/packages/app-core/platforms/electrobun eliza/packages/app-core/src apps/app/test/electrobun-packaged`

## Verification

Use the narrowest real signal:

- Electrobun native tests for launch/env/error formatting.
- Staged desktop build if native packaged files change.
- Fresh state-dir packaged/dev launch if startup behavior changes.

Do not claim startup works without a fresh launch log showing API readiness.
