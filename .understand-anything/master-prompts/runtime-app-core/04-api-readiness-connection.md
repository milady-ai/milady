# API Readiness And Renderer Connection Cleanup
...
Make the renderer/backend connection deterministic: one live API base, one readiness source, and clear failure states.

## Desired Contract

- Fresh install first-run setup reaches the live local API base.
- Packaged desktop tests prove that first-run setup does not get trapped on mock/remote/unready API state.
- Renderer shows loading/skeleton until runtime, API, transcription, and voice readiness gates are satisfied enough for the current screen.
- No duplicate runtime base URL selection paths that disagree between dev and packaged desktop.

## Search First

- `rg -n "apiBase|API_BASE|baseUrl|runtimeBase|health|readiness|startup|StartupShell|FirstRunShell|mockApi|liveApi|localhost|127.0.0.1" apps/app eliza/packages/ui eliza/packages/app-core`

## Verification

- Unit tests for API-base selection.
- Packaged startup/electrobun tests for first-run reaching live API.
- If code changes affect launch, run a fresh state-dir launch and inspect logs before claiming success.
