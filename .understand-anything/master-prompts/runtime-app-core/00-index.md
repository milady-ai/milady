...
0. `00-cross-reference-checklist.md` — required preflight/postflight guardrail for every cleanup slice.
1. `01-boundary-inventory.md` — establish current ownership and exact callers before edits.
2. `02-electrobun-launch-contract.md` — simplify packaged sidecar launch/readiness ownership.
3. `03-state-dir-config-contract.md` — make state-dir/config resolution one contract.
4. `04-api-readiness-connection.md` — make renderer/backend connection deterministic.
5. `05-ui-import-leakage.md` — remove app-core imports from UI-facing code where they violate layering.
6. `06-local-inference-voice-contract.md` — clarify ASR/TTS/model readiness boundary.
7. `07-packaged-startup-verification.md` — prove startup with the narrowest real packaged signal.
8. `08-stale-path-deletion-sweep.md` — delete stale runtime/onboarding names and compat leftovers only with evidence.
9. `09-post-change-diff-review.md` — run graph/diff review after each cleanup slice.

Each prompt is meant to be run as a standalone master prompt in a fresh agent session or pasted into the current session when that slice starts.
