...
/* ── Helpers ─────────────────────────────────────────────────────────── */
function canPortalToBody() {
    return (typeof document !== "undefined" &&
        !!document.body &&
        typeof document.body.appendChild === "function" &&
        !globalThis.__TEST_RENDERER__);
}
...
export function ConfirmDialog({ open, title = "Confirm", message, confirmLabel = "Confirm", cancelLabel = "Cancel", tone = "default", onConfirm, onCancel, }) {
    ...
    if (!open)
        return null;
    const content = (_jsx("div", { className: "fixed inset-0 flex items-center justify-center bg-black/40", style: { zIndex: 10001 }, onClick: (e) => {
...
export function PromptDialog({ open, title = "Enter Value", message, placeholder, defaultValue = "", confirmLabel = "Confirm", cancelLabel = "Cancel", onConfirm, onCancel, }) {
    ...
    const content = (_jsx("div", { className: "fixed inset-0 flex items-center justify-center bg-black/40", style: { zIndex: 10001 }, onClick: (e) => {
            if (e.target === e.currentTarget)
                onCancel();
        }, onKeyDown: handleKeyDown, role: "dialog", "aria-modal": "true", "aria-label": title, tabIndex: -1, children: _jsxs("div", { className: "mx-4 w-full max-w-md rounded-lg border border-border bg-bg p-6 shadow-2xl", children: [_jsx("h2", { className: "mb-3 text-base font-bold", children: title }), _jsx("p", { className: "mb-4 whitespace-pre-line text-sm text-muted", children: message }), _jsx("input", { ref: inputRef, type: "text", value: value, placeholder: placeholder, onChange: (e) => setValue(e.target.value), className: "mb-6 w-full rounded-md border border-border bg-bg-hover px-3 py-2 text-sm" }), _jsxs("div", { className: "flex items-center justify-end gap-3", children: [_jsx("button", { type: "button", onClick: onCancel, className: "rounded-md border border-border px-4 py-2 text-sm transition-colors hover:bg-bg-hover", children: cancelLabel }), _jsx("button", { type: "button", onClick: handleConfirm, className: "rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-fg transition-opacity hover:opacity-90", children: confirmLabel })] })] }) }));
    return renderModalPortal(content);
}
