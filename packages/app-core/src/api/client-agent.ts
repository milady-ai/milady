...
/** Coordinator thread/status queries can exceed the default 10s client timeout (DB, large thread lists). */
const CODING_AGENT_COORDINATOR_TIMEOUT_MS = 30_000;
...
/** `POST /api/agent/restart` waits for full runtime bounce (plugin load, services). */
const AGENT_RESTART_FETCH_TIMEOUT_MS = 120_000;
...
MiladyClient.prototype.restartAgent = async function (this: MiladyClient) {
  const res = await this.fetch<{ status: AgentStatus }>(
    "/api/agent/restart",
    { method: "POST" },
    { timeoutMs: AGENT_RESTART_FETCH_TIMEOUT_MS },
  );
  return res.status;
};
...
MiladyClient.prototype.getCodingAgentStatus = async function (
  ...
  try {
    const status = await this.fetch<CodingAgentStatus>(
      ...
      undefined,
      { timeoutMs: CODING_AGENT_COORDINATOR_TIMEOUT_MS },
    );
    if (
      status &&
    ...
    if (status && !status.tasks) {
      ...
      try {
        const ptySessions = await this.fetch<RawPtySession[]>(
          "/api/coding-agents",
          undefined,
          { timeoutMs: CODING_AGENT_COORDINATOR_TIMEOUT_MS },
        );
