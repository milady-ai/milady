import { Button, cn, Input } from "@miladyai/ui";
...
export function ConnectionElizaCloudPreProviderScreen({
  ...
  const elizaCloudReady =
    elizaCloudConnected ||
    elizaCloudHasPersistedKey ||
    onboardingCloudApiKey.trim().length > 0;
  ...
  return (
    <>
      ...
      <div
        className={`${onboardingFooterClass} relative z-[100]`}
      ...
      >
        ...
        <Button
          className={cn(
            onboardingPrimaryActionClass,
            !elizaCloudReady && "opacity-40 cursor-not-allowed",
          )}
          style={onboardingPrimaryActionTextShadowStyle}
          aria-disabled={!elizaCloudReady}
          onClick={(e) => {
            e.stopPropagation();
            if (!elizaCloudReady) {
              setActionNotice(
                t("onboarding.elizaCloudConfirmBlocked", {
                  defaultValue:
                    "Log in to Eliza Cloud or paste an API key before continuing.",
                }),
                "info",
                5200,
              );
