...
function OrchestratorTasksWidget(_props: ChatSidebarWidgetProps) {
  ...
  useEffect(() => {
    ...
    const refreshThreads = async () => {
      setLoading(true);
      try {
        // Default view: coordinator /status already embeds taskThreads (limit 50).
        // Avoid a second GET /threads (duplicate heavy SQL) when not filtering.
        let nextThreads: Awaited<
          ReturnType<typeof client.listCodingAgentTaskThreads>
        >;
        let nextStatus: Awaited<ReturnType<typeof client.getCodingAgentStatus>>;
        if (!showArchived && !deferredSearch) {
          nextStatus = await client.getCodingAgentStatus();
          if (cancelled) return;
          if (nextStatus?.taskThreads) {
            nextThreads = nextStatus.taskThreads.slice(0, 30);
          } else {
            nextThreads = await client.listCodingAgentTaskThreads({
              includeArchived: false,
              limit: 30,
            });
          }
        } else {
          [nextThreads, nextStatus] = await Promise.all([
