...
import {
  logOnboardingDebug,
  summarizeOnboardingSubmitPayload,
} from "./onboarding-debug";
    ...
    async (options?: OnboardingNextOptions) => {
      ...
      try {
        ...
        if (useCloudFastTrack) {
          ...
          } satisfies Parameters<typeof client.submitOnboarding>[0];

          logOnboardingDebug(
            "submitOnboarding cloud fast track",
            summarizeOnboardingSubmitPayload(
              fastTrackPayload as unknown as Record<string, unknown>,
            ),
          );

          await client.submitOnboarding(fastTrackPayload);
          try {
        ...
        logOnboardingDebug(
          "submitOnboarding standard path",
          summarizeOnboardingSubmitPayload(
            standardPayload as unknown as Record<string, unknown>,
          ),
        );

        await client.submitOnboarding(standardPayload);
        try {
          await persistOnboardingStyleVoice({
            style,
            voiceProvider: onboardingVoiceProvider,
        ...
        logOnboardingDebug("runOnboardingChatHandoff error", err);
        const message =
