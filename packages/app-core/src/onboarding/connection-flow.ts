...
export function isProviderConfirmDisabled(opts: {
  ...
  /**
   * True when GET /api/cloud/status already reports a persisted Eliza Cloud key.
   * React `elizaCloudConnected` can lag behind or stay false briefly after OAuth/persist;
   * without this, Confirm stays disabled even though the backend is ready.
   */
