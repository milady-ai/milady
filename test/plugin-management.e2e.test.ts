...
describe("Plugin Management E2E", () => {
        ...
        mockPluginManager = {
            installPlugin: vi.fn().mockImplementation(async (name) => {
                if (typeof name === "string" && name.includes("non-existent")) {
                    return { success: false, error: "Not found", requiresRestart: false, pluginName: name };
                }
                return { success: true, pluginName: name, requiresRestart: true };
            }),
            ejectPlugin: vi.fn().mockImplementation(async (name) => {
                if (name.includes("non-existent")) return { success: false, error: "Not found", requiresRestart: false, pluginName: name };
                return { success: true, pluginName: name, ejectedPath: "/tmp/ejected", requiresRestart: true };
            }),
            syncPlugin: vi.fn().mockImplementation(async (name) => {
                if (name.includes("non-existent")) return { success: false, error: "Not found", requiresRestart: false, pluginName: name };
                return { success: true, pluginName: name, ejectedPath: "/tmp/ejected", upstreamCommits: 1, requiresRestart: true };
            }),
            reinjectPlugin: vi.fn().mockImplementation(async (name) => {
                if (name.includes("non-existent")) return { success: false, error: "Not found", requiresRestart: false, pluginName: name };
                return { success: true, pluginName: name, removedPath: "/tmp/ejected", requiresRestart: true };
            }),
    ...
    describe("POST /api/plugins/install", () => {
        it("returns 400 for missing name", async () => {
            const { status, data } = await http$(server.port, "POST", "/api/plugins/install", {});
            expect(status).toBe(400);
            expect(data.error).toContain("must include 'name'");
        });

        it("returns 400 for invalid name format", async () => {
            const { status, data } = await http$(server.port, "POST", "/api/plugins/install", { name: "invalid name with spaces" });
            expect(status).toBe(400);
        ...
        it("returns 500/422 for non-existent plugin (verifies service is called)", async () => {
            ...
            const { status, data } = await http$(server.port, "POST", "/api/plugins/install", { name: "@elizaos/non-existent-plugin-12345" });
    ...
    describe("POST /api/plugins/:id/eject", () => {
        it("returns 422/500 for non-installed plugin", async () => {
            const { status, data } = await http$(server.port, "POST", "/api/plugins/non-existent/eject");
    ...
    describe("POST /api/plugins/:id/sync", () => {
        it("returns 422/500 for non-ejected plugin", async () => {
            const { status, data } = await http$(server.port, "POST", "/api/plugins/non-existent/sync");
    ...
    describe("POST /api/plugins/:id/reinject", () => {
        it("returns 422/500 for non-ejected plugin", async () => {
            const { status, data } = await http$(server.port, "POST", "/api/plugins/non-existent/reinject");
            expect(status).not.toBe(404);
            expect(data.error).toBeDefined();
        });
    });
    ...
    describe("GET /api/plugins/ejected", () => {
        it("returns list", async () => {
            const { status, data } = await http$(server.port, "GET", "/api/plugins/ejected");
