...
function cloudClientForToken(token: string | null) {
  if (!token) return null;
  return new CloudClient(token);
}

function webOpenAction(
  cloudPreparing: boolean,
  handleCancelCloudOpen: () => void,
  handleOpenCloud: () => void,
) {
  return cloudPreparing ? handleCancelCloudOpen : handleOpenCloud;
}
...
function cloudButtonLabel(cloudPreparing: boolean) {
  return cloudPreparing ? "cancel opening" : "cloud";
}

function cloudButtonAriaLabel(cloudPreparing: boolean) {
...
function checksumUrl() {
  return releaseData.release.checksum?.url ?? GITHUB_LATEST_RELEASE_URL;
}
...
export function Homepage() {
  ...
  const cloudClient = useMemo(() => cloudClientForToken(token), [token]);
  ...
  const platformLinks = createPlatformLinks({
    cloudPreparing,
    handleCancelCloudOpen,
    handleOpenCloud,
  });
      ...
      <main className="relative z-10 flex min-h-[100dvh] flex-col items-center justify-center px-4 py-28 text-center">
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
          <button
            ...
            aria-label={cloudButtonAriaLabel(cloudPreparing)}
          ...
          >
            {cloudButtonLabel(cloudPreparing)}
          </button>
          <a
