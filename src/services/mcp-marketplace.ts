...
export async function searchMcpMarketplace(
    ...
    try {
        ...
        logger.error("[mcp-marketplace] search error:", err instanceof Error ? err.message : String(err));
        return { results: [] };
    }
}
...
export async function getMcpServerDetails(name: string): Promise<McpRegistryServer | null> {
    try {
        ...
        logger.error("[mcp-marketplace] get server details error:", err instanceof Error ? err.message : String(err));
        return null;
    }
}
